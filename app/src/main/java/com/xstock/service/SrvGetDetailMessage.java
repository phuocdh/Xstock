...
public class SrvGetDetailMessage {

    public static String GetDetailMessage(String token, String id) {

        String strGetDetailMessage = "";

        // Soap config
        final String SOAP_ACTION = AppConfig.SOAP_NAMESPACE + AppConfig.GET_DETAIL_MESSAGE;
        final String METHOD_NAME = AppConfig.GET_DETAIL_MESSAGE;
        ...
        try {
            ...
            PropertyInfo idMsg = new PropertyInfo();
            idMsg.setName("id");
            idMsg.setValue(id);
            idMsg.setType(String.class);
            client.addProperty(idMsg);
            ...
            strGetDetailMessage = jsonArray.getJSONObject(0).getString("Content").toString();

        } catch (Exception e) {
            e.toString();
        }
        return strGetDetailMessage;
    }
}